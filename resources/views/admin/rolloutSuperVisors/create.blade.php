...
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title card_title_center"> {{ __('messages.Add_New') }}  {{ __('messages.rolloutSuperVisors') }} </h3>
        </div>
        <!-- /.card-header -->
        <div class="card-body">


            <form action="{{ route('rolloutSuperVisors.store') }}" method="post" enctype='multipart/form-data'>
                <div class="row">
                    @csrf
